...
class PurchaseManagement(models.Model):
    ...
    overall_total = fields.Float(string='Net Total:', compute='_compute_overall_total', store=True)

    purchase_date = fields.Date(string="Purchase Date")

    line_ids = fields.One2many('purchase.management.line', 'purchase_management_id', string='Product Lines')

    @api.depends('line_ids.total')
    def _compute_overall_total(self):
        for record in self:
            # Sum the total of all vendor.management.line related to this record
            record.overall_total = sum(record.line_ids.mapped('total'))

    @api.model
    def create(self, vals):
        record = super(PurchaseManagement, self).create(vals)
